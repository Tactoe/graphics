...
/*   Updated: 2018/06/04 17:51:07 by tmilon           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		limit_sphere(t_shape shape, t_ray ray, double *t, t_calcunit calc)
{
	...
	min = new_vector_unicoord(-shape.radius);
	max = new_vector(1, 1, 1);
	max = vector_op(max, new_vector_unicoord(shape.radius), '*');
	if (calc.t0 != -1
...
void		limit_cylinder(t_shape shape, t_ray ray, double *t, t_calcunit calc)
{
	...
	calc = limiter_setup(shape, ray, calc);
	min = new_vector(-shape.radius, 0, -shape.radius);
	max = new_vector(1, 1, 1);
	max = vector_op(max, new_vector(shape.radius, shape.height, shape.radius), '*');
	if (calc.t0 != -1
			&& calc.get_in.x >= min.x && calc.get_in.x < max.x
			&& calc.get_in.y >= min.y && calc.get_in.y < max.y
...
void		limit_cone(t_shape shape, t_ray ray, double *t, t_calcunit calc)
{
	...
	calc = limiter_setup(shape, ray, calc);
	min = new_vector(-shape.height, 0, -shape.height);
	max = new_vector(1, 1, 1);
	max = vector_op(max, new_vector_unicoord(shape.height), '*');
	if (calc.t0 != -1
			&& calc.get_in.x >= min.x && calc.get_in.x < max.x
			&& calc.get_in.y >= min.y && calc.get_in.y < max.y
			&& calc.get_in.y < shape.height
